        ...
        for fname, text in (("title", title or ""), ("body", body)):
            ...
            for m in r._re.finditer(text):
                ...
                if r._rq or r._ex:
                    ...
                    ctx = text[lo:hi]
                    if r._rq and not r._rq.search(ctx):
                        continue
                    if r._ex and r._ex.search(ctx):
                        continue
...
# 这三条都是从**真实账号**里实测抓出来的 block 级假警报。
_FP_CASES = [
    ("批评单向沟通，不是要求服从",
...
def _selftest() -> int:
    ...
    print()
    print("== 误报回归（block 级不得裸词） ==")
    for note, text, forbid in _FP_CASES:
        got = {f.rule_id for f in scan("", "<p>" + text + "</p>")}
        hit = got & forbid
        ok = not hit
        print(f"  [{'OK ' if ok else 'FAIL'}] {note}")
        print(f"        不该命中 {sorted(forbid)} / 实得误报 {sorted(hit) or '无'}")
        if not ok:
            bad += 1

    print()
    print(f"回归总计：{len(_PARA_CASES) + len(_FP_CASES) - bad}"
          f"/{len(_PARA_CASES) + len(_FP_CASES)} 通过")
    print()
